...
# 今回は出力層を３つにしてみる。
# { 1, 0, 0 } と { 0, 1, 0 } と { 0, 0, 1 } の３種類の出力を出力する感じ。
...
class Net(nn.Module):
    def __init__(self):
        ...
        self.softmax = torch.nn.Softmax(dim=1)       # dim=1 は、行方向にsoftmaxを適用する。
    ...
    def forward(self, x):
        ...
        x = self.softmax(x) # 活性化関数の使用（softmaxも活性化関数なのか）
        return x
...
optimizer = torch.optim.SGD(net.parameters(), lr=0.3)     # 最適化アルゴリズム
...
compare = outputs.argmax(dim=1) == y_test

print(f'correct: {compare.sum()} / {len(outputs)}')
#for value, label in zip(outputs, y_test):
